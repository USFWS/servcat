...
#' Get the right base URL for the ServCat API
#'
...
.get_base_url <- function(is_secure, is_dev) {
  datastore_url <- dplyr::case_when(
    !is_secure && !is_dev ~ get("sc_public_api", envir = .pkgglobalenv),  # Public API
    !is_secure && is_dev ~ get("sc_dev_public_api", envir = .pkgglobalenv),  # Public testing API (probably limited use cases, but it exists)
    is_secure && !is_dev ~ get("sc_secure_api", envir = .pkgglobalenv),  # Secure API
    is_secure && is_dev ~ get("sc_dev_secure_api", envir = .pkgglobalenv)  # Secure testing API
  )
...
.get_ref_profile_url <- function(ref_id, is_dev) {
  ref_profile_url <- dplyr::case_when(
    is_dev ~ get("sc_dev_reference_url", envir = .pkgglobalenv),
    !is_dev ~ get("sc_reference_url", envir = .pkgglobalenv)
  )
...
#' Create httr2 request for ServCat API
#'
#' @inheritParams .get_base_url
#' @param suppress_errors Suppress HTTP errors? Set to TRUE if using `.validate_resp()`
#'
#' @returns A httr2 request object with curl options set to allow authentication for FWS users (if using secure API)
#' @keywords internal
#'
.servcat_request <- function(is_secure, is_dev, suppress_errors = TRUE) {
...
.get_reference_profiles <- function(reference_ids, is_secure, is_dev) {
  request <- .servcat_request(is_secure = is_secure, is_dev = is_dev) |>
...
#' Retrieve some valid ServCat reference IDs
#'
#' For example and testing purposes. See `?servcat::public_refs and ?servcat::internal_refs` for more information.
#'
...
#'
#' @returns An integer vector of ServCat reference IDs.
#' @export
#'
#' @examples
...
#'
#' Only works for FWS users on the internal network. Accepts a vector of UPNs, a vector of emails, or both.
#'
...
#' @examples
#' \dontrun{
#' upns <- c("john_doe@fws.gov", "jane_doe@fws.gov")
#' emails <- c("john_doe@fws.gov", "jane_doe@fws.gov")
#'
