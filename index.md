...
If you only need summary fields, use
...
[`get_references()`](https://ideal-adventure-2qyoqkw.pages.github.io/reference/get_references.md)
returns a named list because detailed ServCat profile records can
contain nested metadata.
[`get_reference_summaries()`](https://ideal-adventure-2qyoqkw.pages.github.io/reference/get_reference_summaries.md)
returns a tibble.
...
to run a ServCat Advanced Search with one or more search criteria.
Search criteria are sent in the body of a POST request, while paging and
sorting options are sent as query parameters.
...
[`search_references()`](https://ideal-adventure-2qyoqkw.pages.github.io/reference/search_references.md)
returns one page by default. Set `all_pages = TRUE` to retrieve pages
sequentially and combine them into one tibble.
...
to inspect the digital files attached to a reference before downloading
them.
...
When available,
[`get_files()`](https://ideal-adventure-2qyoqkw.pages.github.io/reference/get_files.md)
preserves the API’s `fileSize` field and adds `fileSize_kb` for
convenience.
...
[`download_files()`](https://ideal-adventure-2qyoqkw.pages.github.io/reference/download_files.md)
returns one row per requested file, including the local path, ServCat
resource ID, download link, `success`, and `error`. Failed downloads are
reported in the returned tibble instead of stopping the whole batch.
...
when you need valid reference type values for search criteria.
